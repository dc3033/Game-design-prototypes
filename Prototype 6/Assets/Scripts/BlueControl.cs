...
public class BlueControl : MonoBehaviour {
    ...
    void Start () {
        ...
        yc = yellow.GetComponent<YellowControl>();
	}
	
	void Update () {
		if (grounded && !charging && !stunned && Input.GetKeyDown(KeyCode.W))
        {
        ...
        if (!charging && !stunned && Input.GetKeyDown(KeyCode.LeftShift))
        {
            powering = true;
        ...
        if (grounded && !charging && !stunned && (Input.GetKeyDown(KeyCode.A) || Input.GetKeyDown(KeyCode.D)))
        {
            walk.Play();
        }
    }
    ...
    {
        if (collision.gameObject.CompareTag("yellow") && charging)
        {
            yc.stunned = true;
            yc.stuntime = -1f;
            hit.Play();
